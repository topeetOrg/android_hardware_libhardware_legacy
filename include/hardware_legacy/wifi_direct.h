 ...
 */
int wifi_direct_wait_for_event(char *buf, size_t len);
...
/**
 * wifi_direct_dhcp_client_request() issues a dhcp request and returns the acquired
 * information.
 *
...
/**
 * wifi_direct_start_dhcp_server() starts dhcp server when acting as Group Owner
 *
 * All IPV4 addresses/mask are in network byte order.
 *
 * @param ipaddr_begin begin of IPV4 address to assign
 * @param ipaddr_num number of IPV4 address to assign
 * @param gateway the gateway to assign
 * @param mask IPV4 mask to assign
 * @param dns1 IPV4 address of a DNS server to assign
 * @param dns2 IPV4 address of a DNS server to assign
 * @param lease the length of lease in seconds.
 *
 * @return 0 if successful, < 0 if error.
 */
...
/**
 * wifi_direct_stop_dhcp_server() stops dhcp server when acting as Group Owner
 *
 * @return 0 if successful, < 0 if error.
 */
...
/**
 * wifi_direct_start_nat() starts internet sharing service
 *
 * @return 0 if successful, < 0 if error.
 */
...
/**
 * wifi_direct_stop_nat() stops internet sharing service
 *
 * @return 0 if successful, < 0 if error.
 */
