...
#define WIFI_DRIVER_MODULE_ARG          "suspend_mode=1 debug_mask=0xffffffff" //WLAN_POWER_STATE_CUT_PWR
//#define WIFI_DRIVER_MODULE_ARG          "suspend_mode=2 debug_mask=0x142410" //WLAN_POWER_STATE_DEEP_SLEEP
//#define WIFI_DRIVER_MODULE_ARG          ""
//#define WIFI_DRIVER_MODULE_ARG          "debug_mask=0x1524f0"
#endif
...
static const char WIFI_SDIO_DRIVER_MODULE_NAME[]  = WIFI_SDIO_IF_DRIVER_MODULE_NAME;
static const char WIFI_SDIO_DRIVER_MODULE_PATH[]  = WIFI_SDIO_IF_DRIVER_MODULE_PATH;
static const char WIFI_SDIO_DRIVER_MODULE_ARG[]   = WIFI_SDIO_IF_DRIVER_MODULE_ARG;
#endif
...
#endif
    LOGI("[semco] is_wifi_driver_loaded.");
...
int wifi_load_driver()
{
    ...
    if (insmod(WIFI_SDIO_DRIVER_MODULE_PATH, WIFI_SDIO_DRIVER_MODULE_ARG) < 0)
    	{
        LOGE("[semco] cfg80211 module loading fail.");
        return -1;
    	}
...
int wifi_unload_driver()
{
    ...
    if (rmmod(DRIVER_MODULE_NAME) == 0) {
        ...
        while (count-- > 0) {
            if (!is_wifi_driver_loaded()) {
		  rmmod(WIFI_SDIO_DRIVER_MODULE_NAME);
                break;
            	  }
            usleep(500000);
        }
...
#else
    if (chmod(SUPP_CONFIG_FILE, 0775) < 0) {
        LOGE("Error changing permissions of %s to 0775: %s",
...
{
    LOGI("[semco] wifi_start_p2p_supplicant!");
    //from semco
...
int wifi_stop_supplicant()
{
    ...
    LOGI("[semco] wifi_stop_supplicant!");
...
int wifi_connect_to_supplicant()
{
    char ifname[256];
    char supp_status[PROPERTY_VALUE_MAX] = {'\0'};
    LOGI("[semco] wifi_connect_to_supplicant!");
